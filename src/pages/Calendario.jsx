...
// --- (El resto de las funciones helper y el componente MonthView no cambian) ---
...
const MonthView = ({
  ...
  <div className="grid grid-cols-7">
    {["Lu", "Ma", "Mi", "Ju", "Vi", "Sá", "Do"].map((day) => (
      <div
        key={day}
        className="text-center font-bold text-sm text-muted-foreground p-2 border-b"
      >
        {day}
      </div>
    ))}
    {monthDays.map((day, index) => {
      ...
      const schedule = availability.exceptions[dateKey] ??
        availability.default[dayOfWeek] ?? { available: false, ranges: [] };
...
const Calendario = () => {
      ...
      try {
        const [
          ...
          supabase
            ...
            .lte("appointment_at", monthEnd.toISOString()),
        ...
        const appointmentsWithIndex = (appointmentsRes.data || []).map(
          (appointment) => {
        ...
        // --- CORRECCIÓN: Inicializamos `breaks` en la estructura por defecto ---
        const formattedAvailability = {
          default: {
            0: { available: false, ranges: [], breaks: [] },
            1: { available: false, ranges: [], breaks: [] },
            2: { available: false, ranges: [], breaks: [] },
            3: { available: false, ranges: [], breaks: [] },
            4: { available: false, ranges: [], breaks: [] },
            5: { available: false, ranges: [], breaks: [] },
        ...
        schedulesRes.data.forEach((s) => {
          ...
          if (s.is_break) {
            formattedAvailability.default[dayKey].breaks.push({
              start: s.start_time,
              end: s.end_time,
            });
          } else {
            formattedAvailability.default[dayKey].ranges.push({
              start: s.start_time,
              end: s.end_time,
            });
          }
        ...
        exceptionsRes.data.forEach((e) => {
          formattedAvailability.exceptions[e.exception_date] = {
            available: false,
            ranges: [],
          };
        });
  ...
  const handleSaveAppointment = async (data) => {
    try {
      ...
      const appointmentTimestamp = format(
        appointmentDateTime,
      ...
      const durationInSlots = Math.ceil(
        (insertedAppointment.services?.duration_min || 15) / 15
      );
  ...
  const handleSaveAvailability = async (newAvailability) => {
    setLoading(true);
    try {
      // Borramos la configuración anterior para simplificar
      const { error: deleteSchedulesError } = await supabase
        .from("work_schedules")
        .delete()
        .neq("id", 0); // Borra todas las filas que coincidan con RLS
      ...
      const { error: deleteExceptionsError } = await supabase
        ...
        .delete()
        .neq("id", 0); // Borra todas las filas de excepciones

      if (deleteExceptionsError) throw deleteExceptionsError;

      // Preparamos los nuevos horarios para insertar
      const newSchedules = [];
      ...
      // Preparamos las nuevas excepciones para insertar
      const newExceptions = [];
      ...
      toast({
        ...
        description: "Tus nuevos horarios de trabajo se han guardado.",
      });
    } catch (error) {
  ...
  const title = useMemo(() => {
    if (view === "week" || isMobile) {
      ...
      return `${format(start, "d 'de' MMMM")} - ${format(
        end,
        "d 'de' MMMM 'de' yyyy",
        { locale: es }
      )}`;
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Agenda semanal y mensual interactiva para gestionar citas."
        />
      </Helmet>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="secondary"
                  className="w-auto sm:w-[280px] justify-start text-left font-normal text-lg"
                >
            ...
            {isMobile ? (
              <div className="space-y-4">
                ...
                <MobileDayView
                  ...
                  appointments={appointments.filter((a) =>
                    isSameDay(parseISO(a.appointment_at), currentDate)
                  )}
                  onAppointmentAction={(action, appt) =>
                    handleDeleteAppointment(appt)
                  }
                  onNewAppointment={handleOpenModal}
                  isSlotAvailable={(day, hourIndex) =>
                    getSlotStatus(day, hourIndex) === "available"
                  }
                />
              </div>
            ) : view === "week" ? (
              <div className="premium-card p-0 overflow-hidden">
                <div className="overflow-x-auto">
                  ...
                  >
                    <div
                      className="sticky top-0 z-20 bg-card p-2 border-b border-r"
                    ...
                    {weekDays.map((day, dayIndex) => (
                      <div
                        key={day.toISOString()}
                        className={cn(
                          "sticky top-0 z-20 bg-card p-2 text-center border-b border-r",
                    ...
                    {hours.map((hour, hourIndex) => (
                      <React.Fragment key={hour}>
                        <div
                          className="p-2 border-b border-r text-sm text-center text-muted-foreground flex items-center justify-center"
                        ...
                        {weekDays.map((day, dayIndex) => {
                          ...
                          return (
                            <div
                              ...
                              className={cn(
                                "border-b border-r flex items-center justify-center",
                              ...
                              onClick={() =>
                                status === "available" &&
                                handleOpenModal({ date: day, hourIndex })
                              }
                            >
                              {status === "available" && (
                                <div
                                  className="w-full h-full text-muted-foreground hover:bg-primary/10 hover:text-primary opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center"
                              ...
                              {status === "closed" && (
                                <Moon className="h-5 w-5 text-muted-foreground/30" />
                              )}
                            </div>
                          );
                        })}
                    ...
                    {appointments.map((appointment) => {
                      const dayIndex = weekDays.findIndex((day) =>
                        isSameDay(parseISO(appointment.appointment_at), day)
                      );
      ...
      <ConfirmationDialog
        ...
        title="¿Eliminar Cita?"
        description={`Estás a punto de eliminar la cita de ${
          appointmentToDelete?.clients?.name || "este cliente"
